class DurationPicker{
    ...
    function render()
        <div class="durationpicker-container {settings.classname}">
        <div class="durationpicker-innercontainer">
        { for (item in stage)
            <input min="{settings[stages[item]]['min']}" max="{settings[stages[item]]['max'] }" placeholder="0" type="number" id="duration-{stages[item]}" class="durationpicker-duration" >
            <span class="durationpicker-label"> {settings[stages[item]]['label']}</span>
        </div>
        }
       </div>;
